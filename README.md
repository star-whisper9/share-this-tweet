...
当前项目版本 **v0.5.1** · **[从 AMO 安装](https://addons.mozilla.org/zh-CN/firefox/addon/share-this-tweet/)** · [安装说明](#安装) · [更新记录](CHANGELOG.md)
...
已发布的 0.5.0 中，拼图与卡片均为**静态图片**：GIF 和视频使用已有静态预览，预览不可用时拼接会明确报错，不会省略该项。

0.5.1 测试版提供 **FFmpeg 动态拼图与视频画框**：在设置的实验性功能页开启后，包含 GIF／视频的拼图输出 MP4，动态媒体也可选择上框或下框。可调整输入总大小、时长、像素数和帧率上限，提供快捷配置及 Apple M4 Firefox Desktop 参考耗时计算器，支持进度、诊断下载与手动取消。此功能尚未发布，手机性能仍需实测，规则及验证方法见[动态处理说明](docs/video-processing.md)。卡片仍保持静态。
...
GitHub 安装包的版本号可能显示为 `0.5.0.1` 这样的四段数字，末尾数字用于区分同一版本的签名安装包。
