...
function object(value: unknown): Record<string, unknown> {
  if (typeof value !== 'object' || value === null || Array.isArray(value))
    throw new Error(t('core.source.invalidStructure'));
...
function string(value: unknown, maximum: number): string {
  if (
    ...
    throw new Error(t('core.source.invalidField'));
  return value;
}

function identity(value: unknown): string {
  const result = string(value, 30);
  if (!/^[1-9]\d*$/.test(result)) throw new Error(t('core.source.invalidId'));
  return result;
}
...
export function parseMediaSourceMetadata(value: unknown): MediaSourceMetadata {
  ...
  if (source.schemaVersion !== 1 || source.platform !== 'x')
    throw new Error(t('core.source.unsupportedVersionOrPlatform'));
  ...
  try {
    ...
    throw new Error(t('core.source.invalidTweetUrl'));
  }
  if (
    url.protocol !== 'https:' ||
    url.hostname !== 'x.com' ||
  ...
  if (!/^@[A-Za-z0-9_]{1,15}$/.test(handle))
    throw new Error(t('core.source.invalidPublisherHandle'));
  ...
  if (new TextEncoder().encode(JSON.stringify(result)).length > MAX_MEDIA_SOURCE_BYTES)
    throw new Error(t('core.source.tooLarge'));
  return result;
}
